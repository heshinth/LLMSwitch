...
class Completions:
    ...
    def __init__(
        self, router: Router, client_registry: ClientRegistry,
        circuit_breaker: CircuitBreaker, rate_limit_manager: RateLimitManager
    ):
        self._router = router
    ...
    def create(self, model: str, messages: list, **kwargs) -> Any:
        ...
        for idx, endpoint in enumerate(endpoints):
            ...
            if not limiter.can_accept(req_tokens):
                logger.warning(f"Endpoint for {provider} ({endpoint.model}) skipped: local rate limit exceeded.")
                continue
            ...
            try:
                ...
                if kwargs.get("stream"):
                    return self._stream_wrapper(idx, endpoints, model, messages, req_tokens, **kwargs)
                ...
                if ErrorClassifier.is_rate_limit(e):
                    ...
                    logger.warning(f"Rate limit hit on {provider}. Activating circuit breaker for {cooldown}s.")
    ...
    def _stream_wrapper(
        self, start_idx: int, endpoints: List[Endpoint], model: str, messages: list, req_tokens: int, **kwargs
        ...
        for endpoint in remaining_endpoints:
            ...
            try:
                ...
                limiter.adjust_tokens(req_tokens, actual_prompt_tokens + actual_completion_tokens)
                return
            except OpenAIError as e:
                limiter.adjust_tokens(req_tokens, 0)
...
class Chat:
    ...
    def __init__(
        ...
        self.completions = Completions(router, client_registry, circuit_breaker, rate_limit_manager)
...
class Client:
    ...
    def __init__(self, configs: Union[LLMSwitchConfig, List[LLMSwitchConfig]]):
        ...
        self.chat = Chat(self._router, self._client_registry, self._circuit_breaker, self._rate_limit_manager)
...
class AsyncChat:
    ...
    def __init__(
        ...
        self.completions = AsyncCompletions(router, client_registry, circuit_breaker, rate_limit_manager)
...
class AsyncClient:
    ...
    def __init__(self, configs: Union[LLMSwitchConfig, List[LLMSwitchConfig]]):
        ...
        self.chat = AsyncChat(self._router, self._client_registry, self._circuit_breaker, self._rate_limit_manager)
