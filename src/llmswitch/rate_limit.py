...
class EndpointRateLimiter:
    ...
    def __init__(self, limits: RateLimit):
        self.limits = limits
        self.req_minute_bucket = TokenBucket(limits.rpm, limits.rpm / 60.0) if limits.rpm and limits.rpm > 0 else None
        self.req_day_bucket = TokenBucket(limits.rpd, limits.rpd / 86400.0) if limits.rpd and limits.rpd > 0 else None
        self.tok_minute_bucket = TokenBucket(limits.tpm, limits.tpm / 60.0) if limits.tpm and limits.tpm > 0 else None
        self.tok_day_bucket = TokenBucket(limits.tpd, limits.tpd / 86400.0) if limits.tpd and limits.tpd > 0 else None
    ...
    def can_accept(self, estimated_tokens: int) -> bool:
        ...
        if self.req_day_bucket and not self.req_day_bucket.can_consume(1):
            return False
        if self.tok_minute_bucket and not self.tok_minute_bucket.can_consume(estimated_tokens):
            return False
        if self.tok_day_bucket and not self.tok_day_bucket.can_consume(estimated_tokens):
            return False
        return True
